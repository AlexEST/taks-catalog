...
Components observed:
  ...
  EE-U-EEV  fixed half of the MIXED hybrid (MIXED products skipped)
...
Intentionally not catalogued:
  * MIXED (Paindlik Klops, 50% fixed + 50% spot) — no schema type for a hybrid
...
def parse_payload(j: dict) -> list[dict]:
    ...
    for product in j.get("products", []):
        ...
        if ptype not in ("FIXED", "VARIABLE"):
            continue  # MIXED hybrid: not representable, see module docstring
        name = (bp.get("name") or "").title()
        if not name:
            continue
